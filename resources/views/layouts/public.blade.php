        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="hidden md:flex items-center space-x-8">
                    <a href="{{ url('/') }}" class="text-gray-600 hover:text-utm-primary font-medium transition {{ Request::is('/') ? 'text-utm-primary font-bold' : '' }}">Beranda</a>
                    <a href="{{ route('public.schedule') }}" class="text-gray-600 hover:text-utm-primary font-medium transition {{ Request::routeIs('public.schedule') ? 'text-utm-primary font-bold' : '' }}">Jadwal Global</a>
                </div>
                ...
                <div class="flex items-center gap-3" x-data="{ open: false }">
                    @auth
                        <div class="relative">
                            ...
                            <div x-cloak x-show="open" 
                                ...
                                <a href="{{ route('dashboard') }}" class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 hover:text-utm-primary transition flex items-center gap-2">
                                    <i class="bi bi-speedometer2"></i> Dashboard
                                </a>
                                <a href="{{ route('profile.edit') }}" class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 hover:text-utm-primary transition flex items-center gap-2">
                                    <i class="bi bi-person-gear"></i> Profil Saya
                                </a>
                                <form method="POST" action="{{ route('logout') }}" class="border-t border-gray-100">
                                    @csrf
                                    <button type="submit" class="w-full text-left px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 hover:text-red-700 transition flex items-center gap-2 rounded-b-xl">
                    ...
                    @else
                        ...
                        <a href="{{ route('register') }}" class="px-5 py-2 bg-utm-primary text-white rounded-full font-medium hover:bg-utm-secondary transition shadow-md text-sm">Daftar</a>
                    @endauth
                </div>
            </div>
        </div>
